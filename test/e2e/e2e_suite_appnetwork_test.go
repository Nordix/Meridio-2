...
var ipv6TestCases = []suiteTestCase{
	{
		name:           "Separate App Network IPv6",
		namespace:      "e2e-separate-appnetwork-v6",
...
var _ = Describe("E2E IPv6 Test Suites", Label("ipv6"), func() {
	SetDefaultEventuallyTimeout(5 * time.Minute)
	SetDefaultEventuallyPollingInterval(2 * time.Second)

	for _, suite := range ipv6TestCases {
		suite := suite
		Describe(suite.name, Ordered, func() {
			Context("Deployment", func() {
				for _, gw := range suite.gateways {
			...
			if len(suite.gateways) >= 2 {
				Context("Sidecar Restart Recovery", func() {
					...
					BeforeAll(func() {
						...
						By("capturing initial table IDs")
						cmd := exec.Command("kubectl", "exec", "-n", suite.namespace, targetPod,
							"-c", "network-sidecar", "--", "ip", "-6", "rule", "show")
						out, err := utils.Run(cmd)
						Expect(err).NotTo(HaveOccurred())
					...
					AfterAll(func() {
						...
						var suiteDir string
						switch suite.name {
						case "Separate App Network IPv6":
							suiteDir = "separate-appnetwork-v6"
						default:
							Skip(fmt.Sprintf("Unknown suite: %s", suite.name))
						}
