...
- The `bfd-detection` "suite" is not a standalone topology: it is a kustomize overlay (`suites/bfd-detection/`, no VLAN/ASN of its own) that reuses `separate-appnetwork-v4` and re-patches its two GatewayRouters so `gw-a1-router-v4` keeps BFD and `gw-a2-router-v4` has it removed (both with a shortened 15s hold time). It is used to measure BGP-down detection time (hold time vs BFD) under an abrupt link failure and is therefore mutually exclusive with the `separate-appnetwork-v4` traffic tests. See `suites/bfd-detection/README.md`.
