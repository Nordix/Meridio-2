...
var _ = Describe("Resiliency", Serial, Ordered, func() {
	...
	BeforeAll(func() {
		...
		// Find an LB Pod for the gateway
		pods, err := clientset.CoreV1().Pods(namespace).List(context.Background(), metav1.ListOptions{
			LabelSelector: fmt.Sprintf("gateway.networking.k8s.io/gateway-name=%s", gatewayName),
		})
		Expect(err).NotTo(HaveOccurred())
		Expect(pods.Items).NotTo(BeEmpty(), "No LB Pods found for gateway %s", gatewayName)
	...
	It("recovers after NFQLB process kill", func() {
		...
		By("capturing LB container logs from crashed instance (for debugging)")
