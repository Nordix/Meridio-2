...
func formatBFDInterfaceParams(spec meridio2v1alpha1.BfdSpec) string {
	var params []string
	if spec.MinRx != "" {
		params = append(params, fmt.Sprintf("min rx interval %s;", spec.MinRx))
	}
	if spec.MinTx != "" {
		params = append(params, fmt.Sprintf("min tx interval %s;", spec.MinTx))
	}
	if spec.Multiplier != nil {
		params = append(params, fmt.Sprintf("multiplier %d;", *spec.Multiplier))
	}
