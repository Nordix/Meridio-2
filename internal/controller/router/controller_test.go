...
func TestResolvePasswords(t *testing.T) {
	...
	t.Run("SecretNotFound_SkipsKey", func(t *testing.T) {
		router := newGatewayRouterWithAuth("router1", gwRef, []meridio2v1alpha1.TcpAoKeyChain{
			{KeyId: 1, Algorithm: "hmac-sha-256", SecretName: "missing-secret", SecretKey: "pw"},
		})
		reconciler, _ := setupReconciler(router)
		result := reconciler.resolvePasswords(context.Background(), []*meridio2v1alpha1.GatewayRouter{router})

		assert.Contains(t, result, "router1")
		assert.Empty(t, result["router1"])
	})

	t.Run("SecretKeyNotFound_SkipsKey", func(t *testing.T) {
		secret := newSecret("my-secret", map[string][]byte{
			"other-key": []byte("value"),
		})
		router := newGatewayRouterWithAuth("router1", gwRef, []meridio2v1alpha1.TcpAoKeyChain{
			{KeyId: 1, Algorithm: "hmac-sha-256", SecretName: "my-secret", SecretKey: "password"},
		})
	...
	t.Run("MultipleRouters_DifferentSecrets", func(t *testing.T) {
		...
		router1 := newGatewayRouterWithAuth("router1", gwRef, []meridio2v1alpha1.TcpAoKeyChain{
			{KeyId: 1, Algorithm: "hmac-sha-256", SecretName: "secret-a", SecretKey: "pw"},
		})
		router2 := newGatewayRouterWithAuth("router2", gwRef, []meridio2v1alpha1.TcpAoKeyChain{
			{KeyId: 2, Algorithm: "cmac-aes-128", SecretName: "secret-b", SecretKey: "pw"},
		})
		reconciler, _ := setupReconciler(secret1, secret2, router1, router2)

		result := reconciler.resolvePasswords(context.Background(), []*meridio2v1alpha1.GatewayRouter{router1, router2})
	...
	t.Run("MultipleKeys_PartialFailure", func(t *testing.T) {
		secret := newSecret("my-secret", map[string][]byte{
			"key1": []byte("password-1"),
		})
		router := newGatewayRouterWithAuth("router1", gwRef, []meridio2v1alpha1.TcpAoKeyChain{
			{KeyId: 1, Algorithm: "hmac-sha-256", SecretName: "my-secret", SecretKey: "key1"},
			{KeyId: 2, Algorithm: "hmac-sha-256", SecretName: "my-secret", SecretKey: "key2-missing"},
		})
		reconciler, _ := setupReconciler(secret, router)

		result := reconciler.resolvePasswords(context.Background(), []*meridio2v1alpha1.GatewayRouter{router})

		assert.Equal(t, "password-1", result["router1"][1])
		_, exists := result["router1"][2]
