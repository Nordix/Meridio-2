...
ip addr add fd00:cafe:20::150/64 dev vlan3
ip addr add 200.100.0.101/32 dev vlan3

# VLAN 400 — shared-appnetwork gw-b2 (IPv4) + shared-appnetwork-ds gw-bds2 (dual-stack)
ip link add link eth0 name vlan4 type vlan id 400
ip link set vlan4 up
ip addr add 169.254.21.150/24 dev vlan4
ip addr add fd00:cafe:21::150/64 dev vlan4
...
# VLAN 1500 — bfd-detection gw-bfd
ip link add link eth0 name vlan15 type vlan id 1500
ip link set vlan15 up
ip addr add 169.254.80.150/24 dev vlan15

ethtool -K eth0 tx off

echo "VPN Gateway ready on VLAN 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500"
