...
**BIRD config output**: The controller generates BIRD `authentication ao` blocks with key entries for each keychain item whose Secret is successfully resolved. Keys with missing or unreadable Secrets are skipped (logged as errors) — partial authentication is possible if at least one key resolves.
