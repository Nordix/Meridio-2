...
**14. ~~BGP-learned routes may be delayed up to 60 seconds in kernel routing table~~ (Resolved)**

Fixed in PR #117. The `kernel` protocol blocks now include `scan time` (default 10s, configurable via `--bird-kernel-scan-time`).
